...
export class Addition extends Generic {
    ...
    processing() {
        if(this.second_img.imgWidth == this.image.imgWidth && this.second_img.imgHeight == this.image.imgHeight) {
            for(let i =0; i < this.image.imgSize; i++) {
                if(this.image.red[i] + this.second_img.red[i] > 255) this.image.red[i] = 255;
                else this.image.red[i] += this.second_img.red[i];

                if(this.image.green[i] + this.second_img.green[i] > 255) this.image.green[i] = 255;
                else this.image.green[i] += this.second_img.green[i];

                if(this.image.blue[i] + this.second_img.blue[i] > 255) this.image.blue[i] = 255;
